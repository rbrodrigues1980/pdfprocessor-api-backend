                ...
                .flatMap(tenantId -> {
                    ...
                    return personRepository.findByTenantIdAndCpf(tenantId, normalizedCpf)
                            .switchIfEmpty(Mono.error(
                                    new PersonNotFoundException("Pessoa não encontrada para CPF: " + normalizedCpf)))
                            .flatMap(person -> {
                                ...
                                return readFileContent(filePart)
                                        ...
                                        .flatMap(fileBytes -> {
                                            ...
                                            // Validar tamanho mínimo razoável para um PDF (pelo menos 100 bytes)
                                                                ...
                                                                .switchIfEmpty(
                                                                        ...
                                                                        gridFsService.storeFileWithHash(
                                                                                ...
                                                                                .flatMap(fileId -> {
                                                                                    ...
                                                                                    // 7. Extrair informações da
                                                                                    // declaração (opcional, para
                                                                                    // metadata)
    ...
    private Mono<UploadDocumentResponse> saveDocumentAndUpdatePerson(PayrollDocument document, Person person) {
        return documentRepository.save(document)
                ...
                .flatMap(savedDoc -> {
                    ...
                    log.info("Iniciando processamento automático do documento de IR: {}", savedDoc.getId());
                    ...
                    return ReactiveTenantContext.withTenant(
                            ...
                            .map(processResponse -> {
                                ...
                                return UploadDocumentResponse.builder()
                                        ...
                                        .status(processResponse.getStatus()) // PROCESSING após iniciar processamento
