...
public class DocumentProcessUseCase {
    ...
    private static final Pattern CPF_REGEX_PATTERN = Pattern.compile("(?:CPF|C\\.P\\.F\\.?):?\\s*(\\d{3}\\.\\d{3}\\.\\d{3}-\\d{2})|(\\d{11})", Pattern.CASE_INSENSITIVE);
    private static final Pattern BRUTO_REGEX_PATTERN = Pattern.compile("(?:Total\\s+de\\s+Rendimento[s]?|Rendimentos\\s+Bruto[s]?|Total\\s+Bruto|Bruto\\s+Total|Valor\\s+Bruto|\\bBruto\\b.*?)\\s*:?\\s*(\\d{1,3}(?:\\.\\d{3})*,\\d{2})", Pattern.CASE_INSENSITIVE);
    private static final Pattern DESCONTOS_REGEX_PATTERN = Pattern.compile("(?:Total\\s+de\\s+Desconto[s]?|Total\\s+Desconto[s]?|Total\\s+Descontado|Valor\\s+Descontos|Descontos\\s+Total|\\bDescontos\\b.*?)\\s*:?\\s*(\\d{1,3}(?:\\.\\d{3})*,\\d{2})", Pattern.CASE_INSENSITIVE);
    private static final Pattern LIQUIDO_REGEX_PATTERN = Pattern.compile("(?:Líquido\\s+a\\s+Receber|Valor\\s+Líquido|Líquido\\s+Recebido|Líquido\\s+Receber|Total\\s+Líquido|\\bLíquido\\b.*?)\\s*:?\\s*(\\d{1,3}(?:\\.\\d{3})*,\\d{2})", Pattern.CASE_INSENSITIVE);
     ...
     */
        Mono<PageResult> processPageWithMetadata(PayrollDocument document, byte[] pdfBytes,
                            ...
                            .flatMap(parserResult -> {
                                ...
                                if (parserResult.getEntries().isEmpty()) {
                                    if (aiPdfExtractionService.isEnabled()) {
                                        ...
                                        addWarnEvent(document, pageNumber, ProcessingEventType.TEXT_UNREADABLE,
                                                String.format("Parser regex extraiu 0 rubricas na página %d. Iniciando extração via Gemini AI...", pageNumber),
                                                Map.of("entriesCount", 0));
                                        return saveIntermediateProgress(document)
                                                .then(processPageWithGemini(document, pdfBytes, pageNumber));
                                    } else {
                                        log.warn("\u26A0\uFE0F Parser regex extraiu 0 rubricas na página {}, mas Gemini AI está desabilitado.", pageNumber);
                                        return Mono.just(parserResult);
                                    }
                                }

                                // Tentar extrair CPF e totais do texto para realizar a validação
                                BigDecimal bruto = extractDecimal(pageText, BRUTO_REGEX_PATTERN);
                                BigDecimal descontos = extractDecimal(pageText, DESCONTOS_REGEX_PATTERN);
                                BigDecimal liquido = extractDecimal(pageText, LIQUIDO_REGEX_PATTERN);
                                String cpf = extractString(pageText, CPF_REGEX_PATTERN);

                                boolean canValidate = bruto != null && descontos != null && liquido != null;
                                if (!canValidate) {
                                    log.info("\u2139\uFE0F Totais não extraídos por regex na página {}. Aceitando extração por regex sem validação aritmética.", pageNumber);
                                    addInfoEvent(document, pageNumber, ProcessingEventType.VALIDATION_PASSED,
                                            String.format("Extração via regex na página %d concluída. Totais não encontrados no texto para validação aritmética.", pageNumber),
                                            Map.of("rubricasCount", parserResult.getEntries().size()));
                                    return Mono.just(parserResult);
                                }

                                ValidationResult validation = validationService.validatePayrollExtraction(
                                        parserResult.getEntries(),
                                        bruto,
                                        descontos,
                                        liquido,
                                        cpf,
                                ...
                                log.info("\uD83D\uDCCA Página {} (Parser Regex) - {} rubricas extraídas, score={}, recommendation={}",
                                        pageNumber, parserResult.getEntries().size(),
                                ...
                                if (!validation.isValid()) {
                                    ...
                                    valDetails.put("rubricasCount", parserResult.getEntries().size());
                                    valDetails.put("salarioBruto", bruto);
                                    valDetails.put("totalDescontos", descontos);
                                    valDetails.put("salarioLiquido", liquido);

                                    if (aiPdfExtractionService.isEnabled()) {
                                        log.warn("\u26A0\uFE0F Validação reprovada para parser regex na página {} (score: {}). Iniciando fallback para Gemini AI...", pageNumber, scoreStr);
                                        ...
                                        addWarnEvent(document, pageNumber, ProcessingEventType.VALIDATION_FAILED,
                                                String.format("Validação da extração por regex reprovada na página %d (score: %s), mas Gemini AI está desabilitado. Mantendo extração por regex.", pageNumber, scoreStr),
                                                valDetails);
                                        return Mono.just(parserResult);
                                    }
                                }
                                ...
                                addInfoEvent(document, pageNumber, ProcessingEventType.VALIDATION_PASSED,
                                        String.format("Extração via regex validada com sucesso na página %d (score: %s).", pageNumber, scoreStr),
                                        Map.of("score", score, "recommendation", validation.recommendation(),
                                                "rubricasCount", parserResult.getEntries().size()));
    ...
    private Mono<PageResult> processPageWithGemini(PayrollDocument document, byte[] pdfBytes, int pageNumber) {
        ...
        addInfoEvent(document, pageNumber, ProcessingEventType.GEMINI_EXTRACTION_STARTED,
                String.format("Extração via Gemini AI [%s] iniciada. Aguardando resposta da IA...", modelName),
                Map.of("model", modelName));

        // Salvar progresso intermediário ANTES de iniciar a extração com Gemini
        return saveIntermediateProgress(document)
                .then(aiPdfExtractionService.extractPayrollData(pdfBytes, pageNumber))
                .flatMap(jsonResponse -> {
                    ...
                    if (!validation.isValid()) {
                        ...
                        // Salvar progresso intermediário ANTES de iniciar a cross-validation
                        return saveIntermediateProgress(document)
                                .then(crossValidationService.crossValidate(
                                        pdfBytes, pageNumber,
                                        parsedData.getEntries(),
                                        jsonResponse,
                                        document.getId(),
                                ...
                                .flatMap(crossResult -> {
                                    ...
                                    if (crossResult.requiresManualReview()) {
                                        ...
                                        addWarnEvent(document, pageNumber, ProcessingEventType.ESCALATION_TO_PRO,
                                                String.format("Escalação para Gemini Pro [%s] iniciada. Campos críticos divergentes.", proModelName),
                                                Map.of("reason", "Campos críticos divergentes", "model", proModelName));

                                        long proStart = System.currentTimeMillis();
                                        // Salvar progresso intermediário ANTES de escalar para o Gemini Pro
                                        return saveIntermediateProgress(document)
                                                .then(aiPdfExtractionService.extractPayrollDataWithFallback(pdfBytes, pageNumber))
                                                .map(proJsonResponse -> {
    ...
    private static String extractString(String text, Pattern pattern) {
    ...
    private static BigDecimal extractDecimal(String text, Pattern pattern) {
        String valStr = extractString(text, pattern);
        if (valStr == null) {
            return null;
        }
        String cleanVal = valStr.replace(".", "").replace(",", ".");
        try {
            return new BigDecimal(cleanVal.trim());
        } catch (NumberFormatException e) {
            log.warn("Falha ao converter valor extraído '{}' para BigDecimal: {}", valStr, e.getMessage());
            return null;
        }
    }
}
