...
# Configurações JVM otimizadas para container (Cloud Run = 4 GiB)
...
#
# Distribuição de memória para 4 GiB (4096 MiB):
#   Heap (60% de 4096):   ~2457 MiB  — para objetos da app (PDFBox, Tika, POI, etc.)
#   Metaspace:              max 256 MiB — classes carregadas (Spring + libs pesadas)
#   Code Cache:              64 MiB    — JIT compiled code
#   Direct Memory:          512 MiB    — NIO buffers (WebFlux/upload multipart, MongoDB driver)
#   Thread stacks + tmpfs de spill de upload + margem: ~807 MiB
#   Total estimado:        ~4096 MiB
